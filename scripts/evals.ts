...
for (const [accountId, expectedOutcome] of cases) {
  ...
  if (review.outcome !== expectedOutcome) {
    throw new Error(`${accountId}: expected ${expectedOutcome}, received ${review.outcome}`);
  }

  for (const scorer of scorers) {
    const result = await scorer.run({ input: review, output: review });
    if ((result.score ?? 0) < 0.8) {
      throw new Error(`${accountId}: ${scorer.id} scored ${result.score}`);
    }
  }
}

const atRisk = evaluated.find((review) => review.outcome === 'awaiting_approval');
if (!atRisk) throw new Error('Expected at least one at-risk fixture');

const history = new ReviewHistory(':memory:');
await history.record(atRisk);

if ((await history.previousScore(atRisk.accountId, 'next-run')) !== atRisk.score) {
  throw new Error('Risk-score history did not persist');
}
...
const retryableReview = { ...atRisk, runId: `${atRisk.runId}-retry` };
await history.record(retryableReview);

try {
  await history.writeOnce(retryableReview.runId, () =>
    Promise.reject(new WriteNotAppliedError('Simulated CRM failure')),
  );
  throw new Error('Expected the simulated CRM write to fail');
} catch (error) {
  if (!(error instanceof Error) || error.message !== 'Simulated CRM failure') {
    throw error;
  }
}

await history.writeOnce(retryableReview.runId, () =>
  Promise.resolve({ writeId: 'recovered-write', taskIds: [] }),
);

const dashboard = await history.dashboard(atRisk.accountId);
if (dashboard.reviews !== 2 || dashboard.alerts.length !== 2) {
  throw new Error('Monitoring aggregation failed');
}

console.log(
  `${cases.length} fixtures passed ${scorers.length} evals each plus persistence checks.`,
);
